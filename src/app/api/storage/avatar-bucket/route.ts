...
export async function POST() {
  ...
  const existing = await supabase.storage.getBucket("avatars")
  if (!existing.error && existing.data) {
    return apiSuccess({ ok: true, created: false })
  }

  const created = await supabase.storage.createBucket("avatars", {
    public: true,
  ...
  if (created.error) {
    return apiError(500, "AVATAR_BUCKET_CREATE_FAILED", created.error.message)
  }
